...
More information on the Flux bootstrap process can be found [here](https://fluxcd.io/flux/cmd/flux_bootstrap_github/)
...
Once the above steps have been completed, you can now commit and push the changes to the repository and merge into the main branch.

### Step 9

**Once the data pipelines have been deployed with the containters deployed to the Azure Container Registry, you need to update the following values in the `dpn-flux-deployment/infrastructure/data-pipelines/releases.yaml` file.**
...
- Find and replace `[DATA_PRODUCT]` with the data product name

Within the `clusters/CHANGE_TO_CLUSTER_NAME/platform.yaml` file, you will need to remove the comments from line 124 to 136. and merge the changes into the main branch, this will deploy the data pipelines to the AKS cluster.
